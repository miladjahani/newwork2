...
en: {
 ...
 step_config_t:'configure', step_config_d:'name, domain, integrations',
 ...
 ey_target:'target', h_account:'Choose the account', lede_account:'The worker and its D1 database will be created under this account.',
 ...
 method_workers:'Cloudflare Workers', method_workers_d:'Native D1 binding. One-click, robust.',
 ...
 adv_feat:'optional · telegram bot & fallback relays',
 f_tg_label:'Telegram bot token — optional', f_tg_ph:'123456:ABC-token-from-@BotFather', f_tg_help:'users can message the bot to fetch their subscription link and usage. Leave empty to configure later from the panel\'s Settings.',
 f_relay_label:'Regional fallback relays — optional', f_relay_ph:'1.2.3.4:443:DE', f_relay_help:'one per line: address:port:REGIONCODE. Used automatically if a direct connection to a destination fails.',
 ...
 res_title:'Live on the edge 🎉', res_lede:'Your panel is deployed. Open it and set your admin password on first visit.',
 res_base:'worker base url', res_panel:'panel link', copy:'copy', open_panel:'open panel',
 res_note:'Set a strong password the first time you open the panel — until then, its API is open to anyone with the URL.',
 ...
 l_source:'Fetching worker source', l_kv:'Creating D1 database', l_upload:'Uploading & deploying script', l_warmup:'Initializing database schema', l_tg:'Registering Telegram bot', l_relay:'Saving fallback relays',
 l_devroute:'Enabling workers.dev route', l_subdomain:'Reading workers.dev subdomain',
 l_proj:'Creating Pages project', l_bind:'Binding D1 & variables to project', l_pages_dep:'Uploading _worker.js deployment', l_cdom:'Attaching custom domain',
...
fa: {
 ...
 step_config_t:'پیکربندی', step_config_d:'نام، دامنه، یکپارچه‌سازی‌ها',
 ...
 ey_target:'هدف', h_account:'حساب را انتخاب کن', lede_account:'ورکر و پایگاه‌داده D1 آن زیر این حساب ساخته می‌شوند.',
 ...
 method_workers:'Cloudflare Workers', method_workers_d:'اتصال بومی D1. یک‌کلیکی و مطمئن.',
 ...
 adv_feat:'اختیاری · ربات تلگرام و آدرس‌های جایگزین',
 f_tg_label:'توکن ربات تلگرام — اختیاری', f_tg_ph:'123456:ABC-توکن-از-@BotFather', f_tg_help:'کاربران می‌توانند با پیام دادن به بات، لینک اشتراک و مصرف خود را بگیرند. اگر خالی بگذاری، بعداً از تنظیمات پنل قابل فعال‌سازی است.',
 f_relay_label:'آدرس‌های جایگزین منطقه‌ای — اختیاری', f_relay_ph:'1.2.3.4:443:DE', f_relay_help:'هر خط: آدرس:پورت:کدمنطقه. اگر اتصال مستقیم به مقصد شکست بخورد، خودکار استفاده می‌شود.',
 ...
 res_title:'روی لبه زنده شد 🎉', res_lede:'پنل تو مستقر شد. بازش کن و در اولین ورود، رمز عبور ادمین را تنظیم کن.',
 res_base:'آدرس پایهٔ ورکر', res_panel:'لینک پنل', copy:'کپی', open_panel:'باز کردن پنل',
 res_note:'در اولین بازدید از پنل حتماً یک رمز قوی تنظیم کن — تا قبل از آن، API پنل برای هرکسی که لینک را دارد باز است.',
 ...
 l_source:'دریافت منبع ورکر', l_kv:'ساخت پایگاه‌داده D1', l_upload:'آپلود و استقرار اسکریپت', l_warmup:'مقداردهی اولیه دیتابیس', l_tg:'ثبت ربات تلگرام', l_relay:'ذخیره آدرس‌های جایگزین',
 l_devroute:'فعال‌سازی مسیر workers.dev', l_subdomain:'خواندن زیردامنهٔ workers.dev',
 l_proj:'ساخت پروژهٔ Pages', l_bind:'اتصال D1 و متغیرها به پروژه', l_pages_dep:'آپلود استقرار _worker.js', l_cdom:'اتصال دامنهٔ سفارشی',
...
var SCOPES = [
 {key:'account.d1.edit',            scope:'Account', resource:'D1', effect:'Edit', req:true, id:'', en:'Account · D1 · Edit', fa:'حساب · D1 · Edit'},
...
var S = {
 ...
 scriptName: '', tgToken: '', fallbackRelays: '', customDomain: '', zoneId: '',
...
/* ---- deploy ---- */
function buildFormData(code, dbId){
  var bindings = [
    { type:'d1', name:'DB', id: dbId }
  ];
  if (S.token) bindings.push({ type:'secret_text', name:'CF_API_TOKEN', text: S.token });
  if (S.accountId) bindings.push({ type:'plain_text', name:'CF_ACCOUNT_ID', text: S.accountId });
  if (S.scriptName) bindings.push({ type:'plain_text', name:'WORKER_NAME', text: S.scriptName });
  var meta = {
...
/* Runs a single SQL statement against the freshly created D1 database,
   through the same /__cf same-origin proxy used for the rest of the
   Cloudflare API — this lets us pre-configure the Telegram bot / fallback
   relays at install time without ever calling the deployed worker itself
   (avoids any cross-origin concerns). */
function d1Query(acc, dbId, sql, params){
  return api('POST', '/accounts/'+acc+'/d1/database/'+dbId+'/query', { sql: sql, params: params || [] });
}
function d1SetSetting(acc, dbId, key, value){
  return d1Query(acc, dbId, "INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)", [key, value]);
}
...
function deploy(){
  ...
  var code, dbId, baseUrl;
  var relays = parseFallbackRelays(S.fallbackRelays);

  Promise.resolve().then(function(){
    var d = timed('l_source'); return fetchSource().then(function(c){ code=c; d(); log('   ' + code.length + ' bytes · module worker', 'ok'); });
  }).then(function(){
    var d = timed('l_kv');
    return api('POST', '/accounts/'+acc+'/d1/database', { name: name + '-db' }).then(function(r){ dbId=r.result.uuid; d(); });
  }).then(function(){
    if (S.method === 'workers') return deployWorkers(code, dbId, name, acc).then(function(u){ baseUrl=u; });
    return deployPages(code, dbId, name, acc).then(function(u){ baseUrl=u; });
  }).then(function(){
    ...
    // Warm up: the very first request to the worker creates the schema
    // (users/settings tables) inside the freshly created, empty D1 database.
    // We need that to exist before writing telegram/relay settings below.
    if (!S.tgToken && !relays.length) return;
    var d = timed('l_warmup');
    var warmBase = S.customDomain ? ('https://' + S.customDomain) : baseUrl;
    return fetch(warmBase + '/').catch(function(){}).then(function(){ d(); });
  }).then(function(){
    if (!S.tgToken) return;
    var d = timed('l_tg');
    var webhookUrl = (S.customDomain ? ('https://' + S.customDomain) : baseUrl) + '/api/tg/webhook';
    var secret = genUuid().replace(/-/g,'');
    return fetch('https://api.telegram.org/bot'+S.tgToken+'/setWebhook', {
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ url: webhookUrl, secret_token: secret })
    }).then(function(r){ return r.json(); }).then(function(tgRes){
      if (!tgRes || !tgRes.ok) { removeCaret(); log('⚠ telegram webhook: ' + (tgRes && tgRes.description || 'failed') + ' (می‌توانید بعداً از تنظیمات پنل دوباره امتحان کنید)', 'warn'); d(); return; }
      return fetch('https://api.telegram.org/bot'+S.tgToken+'/getMe').then(function(r){ return r.json(); }).then(function(meRes){
        var botUsername = (meRes && meRes.ok && meRes.result.username) || '';
        return Promise.all([
          d1SetSetting(acc, dbId, 'tg_bot_token', S.tgToken),
          d1SetSetting(acc, dbId, 'tg_bot_secret', secret),
          d1SetSetting(acc, dbId, 'tg_bot_username', botUsername),
          d1SetSetting(acc, dbId, 'tg_bot_enabled', '1')
        ]);
      }).then(function(){ d(); log('✓ telegram bot connected', 'ok'); });
    }).catch(function(e){ removeCaret(); log('⚠ telegram setup: ' + (e && e.message || e), 'warn'); d(); });
  }).then(function(){
    if (!relays.length) return;
    var d = timed('l_relay');
    return d1SetSetting(acc, dbId, 'fallback_relays', JSON.stringify(relays)).then(function(){ d(); })
      .catch(function(e){ removeCaret(); log('⚠ fallback relays: ' + (e && e.message || e), 'warn'); d(); });
  }).then(function(){
    log('✓ ' + T('l_done'), 'ok');
    setSig('online', T('sig_online'));
    var finalBase = S.customDomain ? ('https://' + S.customDomain) : baseUrl;
    showResult(finalBase, finalBase + '/panel', dbId);
...
function parseFallbackRelays(raw){
  if (!raw) return [];
...
function deployWorkers(code, dbId, name, acc){
  var d = timed('l_upload');
  return api('PUT', '/accounts/'+acc+'/workers/scripts/'+name, buildFormData(code, dbId)).then(function(){ d(); })
  .then(function(){
...
function deployPages(code, dbId, name, acc){
  var d = timed('l_proj');
  ...
  .then(function(){
    ...
    var cfg = { deployment_configs:{ production:{
      ...
      environment_variables: (function(){
        var vars = {};
        if (S.token) vars.CF_API_TOKEN = { value:S.token, type:'secret_text' };
        if (S.accountId) vars.CF_ACCOUNT_ID = { value:S.accountId, type:'plain_text' };
        if (S.scriptName) vars.WORKER_NAME = { value:S.scriptName, type:'plain_text' };
        return vars;
      })()
    }}};
...
function showResult(base, panel, dbId){
  var slot=$('#resultSlot');
  slot.innerHTML =
   '<div class="result"><span class="ring"></span>'+
     ...
     '</div>'+
     '<div class="meta"><span>'+S.method+'</span><span>'+escapeHtml(S.scriptName)+'</span><span>D1 '+escapeHtml(dbId.slice(0,8))+'…</span></div>'+
...
function saveCfg(){
  localStorage.setItem('ef_cfg', JSON.stringify({ scriptName:S.scriptName, customDomain:S.customDomain, sourceUrl:S.sourceUrl, method:S.method }));
}
function loadCfg(){
...
function init(){
  ...
  $('#tgToken').oninput=function(){ S.tgToken=this.value.trim(); };
  $('#fallbackRelays').oninput=function(){ S.fallbackRelays=this.value; };

  $('#act4').onclick = function(){
    clearErr('nameErr'); clearErr('domainErr');
...
// Pro flow: pre-scoped token covering Workers Scripts, Workers KV, Pages, DNS,
// Zone Settings (all Edit) + User Details (Read) — needed once D1/KV multi-tenant
// features and custom-domain DNS management are in use.
var CF_TOKENS_PAGE_PRO = 'https://dash.cloudflare.com/profile/api-tokens?permissionGroupKeys=%5B%7B%22key%22%3A%22workers_scripts%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22d1%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22page%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22dns%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22zone_settings%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22user_details%22%2C%22type%22%3A%22read%22%7D%5D&accountId=*&zoneId=all&name=BPB-Wizard-Pro';
...
function buildPrefillUrl(){
  ...
  // First visit: no self-learned permission-group IDs yet, so fall back to the
  // pre-scoped Pro URL (covers Workers Scripts/KV, Pages, DNS, Zone Settings, User Details).
  // Once hydrateScopeIds() learns the account's actual IDs, later visits use those instead.
  if (!ids.length) return CF_TOKENS_PAGE_PRO;
